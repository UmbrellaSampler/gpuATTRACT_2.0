...
public:
	...
	void swapBuffers() {
	//	std::cout << pipeIdxDof[0] <<pipeIdxDof[1] <<pipeIdxDof[2] << std::endl;
...
//				sumy += h_potLig.getY()[i];
//				sumz += h_potLig.getZ()[i];
//			}
...
//				it->resultBuffer());
	...
	size_t dofSizeRec = 13 + 5;// + Common_Modes::numModesRec;
	size_t dofSizeLig = 13 + 5;//+ Common_Modes::numModesLig;
...
auto GPUEnergyService6DModes<REAL>::createItemProcessor() -> itemProcessor_t {
	...
	itemProcessor_t fncObj = [this, deviceId, p] (workItem_t* item) -> bool {
		...
		if (item != nullptr) {
			...
			const unsigned& dofSizeRec = 13+ 5 ;//+ Common_Modes::numModesRec;
			const unsigned& dofSizeLig= 13+ 5 ;//+ Common_Modes::numModesLig;
