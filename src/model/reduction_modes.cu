...
__global__ void
blockReduceMode(unsigned numAtoms, unsigned numModesRec, unsigned numModesLig, typename Types_6D_modes<T>::DOF* dofs,
		T* xPos, T* yPos, T* zPos,
		T *xModesRec,T *yModesRec,T *zModesRec,
		T *xModesLig,T *yModesLig,T *zModesLig,
		T *d_fxRec, T *d_fyRec, T *d_fzRec,
		T *d_fxLig, T *d_fyLig, T *d_fzLig, T *d_E,
		T *g_odata)
{
    ...
    for(int mode=0;mode<numModesLig;mode++){ sdata[tid + (13+mode)* blockSize] =  sum_modeLig[mode];}
    __syncthreads();
    ...
    if (tid == 0) {
    	g_odata[0  + blockIdx.x*(13+numModesLig)] = sum_fx;
    	g_odata[1  + blockIdx.x*(13+numModesLig)] = sum_fy;
    	g_odata[2  + blockIdx.x*(13+numModesLig)] = sum_fz;
    	g_odata[3  + blockIdx.x*(13+numModesLig)] = sum_E;
    	g_odata[4  + blockIdx.x*(13+numModesLig)] = sum_torque[0];
    	g_odata[5  + blockIdx.x*(13+numModesLig)] = sum_torque[1];
    	g_odata[6  + blockIdx.x*(13+numModesLig)] = sum_torque[2];
    	g_odata[7  + blockIdx.x*(13+numModesLig)] = sum_torque[3];
    	g_odata[8  + blockIdx.x*(13+numModesLig)] = sum_torque[4];
    	g_odata[9  + blockIdx.x*(13+numModesLig)] = sum_torque[5];
    	g_odata[10 + blockIdx.x*(13+numModesLig)] = sum_torque[6];
    	g_odata[11 + blockIdx.x*(13+numModesLig)] = sum_torque[7];
    	g_odata[12 + blockIdx.x*(13+numModesLig)] = sum_torque[8];
    	for(int mode=0;mode<numModesLig;mode++){
    	g_odata[13 + mode + blockIdx.x*(13+numModesLig)] =  sum_modeLig[mode];}
    }
}
		...
		const cudaStream_t& stream)
{
	...
	// when there is only one warp per block, we need to allocate two warps
	// worth of shared memory so that we don't index shared memory out of bounds
	const int smemSize = (threads <= 32) ? 2 * (13 + numModesLig) * threads * sizeof(T) : (13 + numModesLig) * threads * sizeof(T);
	...
	if (isPow2(numAtoms))
	{
		...
		{
			case 1024:
				blockReduceMode<T, 1024,true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 512:
				blockReduceMode<T, 512, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 256:
				blockReduceMode<T, 256, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 128:
				blockReduceMode<T, 128, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 64:
				blockReduceMode<T,  64, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 32:
				blockReduceMode<T,  32, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 16:
				blockReduceMode<T,  16, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  8:
				blockReduceMode<T,   8, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  4:
				blockReduceMode<T,   4, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  2:
				blockReduceMode<T,   2, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  1:
				blockReduceMode<T,   1, true><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;
		}
	}
	else
	{
		...
		{
			case 1024:
				blockReduceMode<T, 1024,false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 512:
				blockReduceMode<T, 512, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 256:
				blockReduceMode<T, 256, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 128:
				blockReduceMode<T, 128, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 64:
				blockReduceMode<T,  64, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 32:
				blockReduceMode<T,  32, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case 16:
				blockReduceMode<T,  16, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  8:
				blockReduceMode<T,   8, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  4:
				blockReduceMode<T,   4, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  2:
				blockReduceMode<T,   2, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;

			case  1:
				blockReduceMode<T,   1, false><<< dimGrid, dimBlock, smemSize, stream >>>(numAtoms, numModesLig, dofs, xPos, yPos, zPos, xModesLig, yModesLig, zModesLig, d_fx, d_fy, d_fz, d_E, g_odata);
				break;
		}
	}
