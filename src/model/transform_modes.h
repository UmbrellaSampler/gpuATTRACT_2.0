...
DOF_6D_Modes<REAL> invertDOF(const DOF_6D_Modes<REAL> &receptorDOF,const DOF_6D_Modes<REAL> ligandDOF){
	DOF_6D_Modes<REAL> invertedDOF;
	invertedDOF.ang=receptorDOF.ang+ligandDOF.ang.inv();
	invertedDOF.pos=receptorDOF.pos+ligandDOF.pos.inv();
	const RotMat<REAL> rotMat=euler2rotmat(invertedDOF.ang.x, invertedDOF.ang.y, invertedDOF.ang.z);
	invertedDOF.pos=rotMat*invertedDOF.pos;
return invertedDOF;
}
