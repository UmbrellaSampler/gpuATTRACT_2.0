...
vector<po::options_description> TwoBodyParser::options() const noexcept {
	...
	input.add_options()
			...
			("receptor-alphabet,a", po::value<string>()->default_value("receptorr.pdb")		, "receptor grid alphabet file");
...
void TwoBodyParser::assigneArgs(po::variables_map const& vm) noexcept {
	...
	if(vm.count("receptor-alphabet"))
		_args->recGridAlphabetName = vm["receptor-alphabet"].as<string>();
	if(vm.count("numCPUs"))
